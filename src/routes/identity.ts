...
export function identityRoutes(app: FastifyInstance) {
  ...
  app.post("/identity/resolve-effective-user", async (request, reply) => {
    ...
    if (!sessionUserId) return reply.code(400).send({ error: "sessionUserId é obrigatório" });
  ...
  app.post("/identity/auto-baixa-diario", async (request, reply) => {
    const { userId, enabled } = request.body as { userId?: string; enabled?: boolean };
    if (!userId || typeof enabled !== "boolean") {
      return reply.code(400).send({ error: "userId e enabled são obrigatórios" });
    }
    try {
